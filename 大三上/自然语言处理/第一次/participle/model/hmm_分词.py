...
def text_to_state(file="../data/hmm/all_train_text.txt"):
    all_data = open(file, "r", encoding="utf-8").read().split("\n")

    with open("../data/hmm/all_train_state.txt", "w", encoding="utf-8") as f:
...
class HMM:
    def __init__(self,file_text = "../data/hmm/all_train_text.txt",file_state = "../data/hmm/all_train_state.txt"):
...
#训练计算三大概率矩阵
    def train(self):
        if os.path.exists("../data/hmm/three_matrix.pkl"):
                self.init_matrix, self.transfer_matrix, self.emit_matrix =  pickle.load(open(
                    "../data/hmm/three_matrix.pkl", "rb"))
                return
        ...
        self.normalize()
        pickle.dump([self.init_matrix, self.transfer_matrix, self.emit_matrix], open("../data/hmm/three_matrix.pkl", "wb"))
#测试阶段维特比算法求出概率最大路径
